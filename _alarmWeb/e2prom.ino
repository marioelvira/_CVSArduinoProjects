...
void _readCONFIG (void)
{
  ...
  if (i != EEPROM_VAL_OK)
  {
    ...
    */
    ...
    EEPROM.write(EEPROM_ADD_TIMEANTI_IN0, EEPROM_VAL_TIMEANTI_IN0);
    EEPROM.write(EEPROM_ADD_LOGIG_INS,    EEPROM_VAL_LOGIG_INS);
    EEPROM.write(EEPROM_ADD_LOGIG_OUTS,   EEPROM_VAL_LOGIG_OUTS);
  ...
  */
  ...
  // Timers
  cfgTimeoutSec = (unsigned long)EEPROM.read(EEPROM_ADD_TIMEOUT_IN0);
  cfgTimeAntiSec = (unsigned long)EEPROM.read(EEPROM_ADD_TIMEANTI_IN0);
  cfgLogicIns = (unsigned long)EEPROM.read(EEPROM_ADD_LOGIG_INS);
  cfgLogicOuts = (unsigned long)EEPROM.read(EEPROM_ADD_LOGIG_OUTS);

  #if (_EEPROM_SERIAL_DEBUG_ == 1)
  Serial.print("Time Out Aux1: ");   Serial.println(inTimeoutSec);
  #endif
}

...
void _serveMAIN()
{
  ...
  html = html + "<title>CHARGER+ Estado</title>";
  html = html + "<head>";
  ...
  html = html + "<h1>CHARGER+ #Estado<span>ESP8266 tech</span></h1>";
...
void _serveTimeSETTINGS()
{
  ...
  html = "<!DOCTYPE HTML><html>";
  html = html + "<title>CHARGER+ #Configuraci&oacuten</title>";
  html = html + "<head>";
  html = html + "<meta http-equiv=\"Content-Type\" content=\"text/html; charset=UTF-8\"/>";
  html = html + "<link rel=\"icon\" href=\"data:,\">";
  html = html + "<link rel=\"stylesheet\" type=\"text/css\" href=\"style.css\" />";
  ...
  html = html + "<body>";
  html = html + "<div class=\"myform\">";
  html = html + "<h1>CHARGER+ #Configuraci&oacuten<span>ESP8266 tech</span></h1>";
  ...
  html = html + "<label>Tiempo espera Entrada (segundos)<input type=\"text\"  maxlength=\"16\" value=\"" + String(inTimeoutSec) + "\" name=\"time0\"/></label>";
...
void _setTimeSETTINGS()
{
  ...
  if (error == 0)
  {
    inTimeoutSec = rtime0.toInt();
    
    #if (_HTTP_SERIAL_DEBUG_ == 1)
    Serial.print("Timeout0 (sec): "); Serial.println(inTimeoutSec);
    #endif
    EEPROM.write(EEPROM_ADD_TIMEOUT_IN0, (byte)inTimeoutSec);
...
void _serveSETTINGS()
{
  ...
  html = html + "<body>";
  html = html + "<div class=\"myform\">";
  html = html + "<h1>CHARGER+ #Network settings<span>ESP8266 tech</span></h1>";

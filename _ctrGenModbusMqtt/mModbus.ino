#include "main.h"

#if (_USE_MB_ == 1)

/**********************************************************************
*    6.-  MODULE PRIVATE VARIABLES.
***********************************************************************/
static const char aucCRCHi[] =
{
    
};

static const char aucCRCLo[] = {
    
};

/////////////////////
// Local Functions //
/////////////////////
void _mbCRC(void)
{ 
  char  ucCRCHi = 0xFF;
  char  ucCRCLo = 0xFF;
  int   iIndex;
  int   tmpChar;
  int   sizeFrame = mrs485TxNumBytes - (int)CRCSIZE;
  
  for (int i=0; i<sizeFrame; i++)
  {
    tmpChar = (char)mrs485TxBuffer[i];
    iIndex = ucCRCLo ^ ( tmpChar & 0x00FF );
    ucCRCLo = (char)( ucCRCHi ^ aucCRCHi[iIndex] );
    ucCRCHi = aucCRCLo[iIndex];
  }

  mbCRC[1] = (ucCRCHi & 0xFF);   // highbyte
  mbCRC[0] = (ucCRCLo & 0xFF);   // lowbyte
}

// Tx: FF 02 00 00 00 08 6C 12
// Rx: FF 02 01 51 AO
void _mbReadIns(void)
{
  // Num Bytes
  mrs485TxNumBytes = 8;

  mrs485TxBuffer[0] = 0xFF;
  mrs485TxBuffer[1] = 0x02;
  mrs485TxBuffer[2] = 0x00;
  mrs485TxBuffer[3] = 0x00;
  mrs485TxBuffer[4] = 0x00;
  mrs485TxBuffer[5] = 0x08;
  // Crc
  _mbCRC();
  mrs485TxBuffer[6] = mbCRC[0];
  mrs485TxBuffer[7] = mbCRC[1];

  // Init RS485 Transmit
  mrs485State = MRS485_INITTX;
}

// Rx: FF 02 01 51 A0
int _mbAnalyseIns(void)
{
  int error = 0;

  if ((mrs485RxBuffer[0] == 0xFF) &&
      (mrs485RxBuffer[1] == 0x02))
  {
    // check CRC (TODO)
  }
  else
    error = 1;

  return error;
}

void _mbUdateIns (void)
{
  if (mrs485RxBuffer[2] & 0x01)
    mbIns[0] = IO_ON;
  else
    mbIns[0] = IO_OFF;

  if (mrs485RxBuffer[2] & 0x02)
    mbIns[1] = IO_ON;
  else
    mbIns[1] = IO_OFF;

  if (mrs485RxBuffer[2] & 0x04)
    mbIns[2] = IO_ON;
  else
    mbIns[2] = IO_OFF;

  if (mrs485RxBuffer[2] & 0x08)
    mbIns[3] = IO_ON;
  else
    mbIns[3] = IO_OFF;

  if (mrs485RxBuffer[2] & 0x10)
    mbIns[4] = IO_ON;
  else
    mbIns[4] = IO_OFF;

  if (mrs485RxBuffer[2] & 0x20)
    mbIns[5] = IO_ON;
  else
    mbIns[5] = IO_OFF;

  if (mrs485RxBuffer[2] & 0x40)
    mbIns[6] = IO_ON;
  else
    mbIns[6] = IO_OFF;

  if (mrs485RxBuffer[2] & 0x80)
    mbIns[7] = IO_ON;
  else
    mbIns[7] = IO_OFF;
}

// Tx: FF 01 00 00 00 08 28 12
// Rx: FF 01 01 A1 AO
void _mbReadOuts(void)
{
  // Num Bytes
  mrs485TxNumBytes = 8;

  mrs485TxBuffer[0] = 0xFF;
  mrs485TxBuffer[1] = 0x01;
  mrs485TxBuffer[2] = 0x00;
  mrs485TxBuffer[3] = 0x00;
  mrs485TxBuffer[4] = 0x00;
  mrs485TxBuffer[5] = 0x08;
  _mbCRC();
  mrs485TxBuffer[6] = mbCRC[0];
  mrs485TxBuffer[7] = mbCRC[1];
  
  // Init RS485 Transmit
  mrs485State = MRS485_INITTX;
}

// Tx: FF 05 00 01 FF 00 C8 24 - Rele 2 a ON
// Rx: FF 05 00 01 FF 00 C8 24
// out: 0 to 7; 8 all relays
// val: 0 OFF, 1 ON
void _mbWirteOut(int out, int val)
{
  // Num Bytes
  mrs485TxNumBytes = 8;
      
  mrs485TxBuffer[0] = 0xFF;
  mrs485TxBuffer[1] = 0x05;
  mrs485TxBuffer[2] = 0x00;
  mrs485TxBuffer[3] = out;
  if (val == 0)
    mrs485TxBuffer[4] = 0x00;
  else
    mrs485TxBuffer[4] = 0xFF;
  mrs485TxBuffer[5] = 0x00;
  // Crc
  _mbCRC();
  mrs485TxBuffer[6] = mbCRC[0];
  mrs485TxBuffer[7] = mbCRC[1];

  // Init RS485 Transmit
  mrs485State = MRS485_INITTX;
}

////////////////////
// mModbus set up //
////////////////////
void _MBSetup(void)
{
  mbState = MB_STANDBY;
  mbTick = millis();

  // Modbus DIOs reset
  for (int i = 0; i < MB_NUM_IOS; i++)
  {
    mbIns[i] = 0;
    mbOuts[i] = 0;
  }
}

//////////////////
// mModbus Loop //
//////////////////
void _MBLoop(void)
{
  int j;
  
  switch (mbState)
  {
    case MB_STANDBY:
      
      if (millis() - mbTick >= 5000)
        mbState = MB_READINS;
    
	    break;

    case MB_READINS:
      _mbReadIns();

      // Analyse Response
      mbTick = millis();
      mbState = MB_INSSTATUS;
      
      break;

    case MB_INSSTATUS:

      // wait for response
      if (millis() - mbTick >= 5000) // 1000 TODO
      {
        //Serial.println("Time out Rx");       
        mbTick = millis();
        mbState = MB_STANDBY;
      }

      // if response received
      if (mrs485State == MRS485_FRAME_RX)
      {
        // Analyse response
        if (_mbAnalyseIns() == MB_RX_OK)
         _mbUdateIns();
        //else
          // Error;

        mbTick = millis();
        mbState = MB_STANDBY;
                
        // clear Rx buffer
        mrs485RxBuffer = "";
        mrs485State = MRS485_STANDBY;
      }

      break;

    /*
    case MB_WRITEOUTS:

      //_mbWirteOut(MB_OUT1, MB_OUT_OFF);
      //_mbWirteOut(MB_OUT1, MB_OUT_ON);
            
      // Analyse Response
      mbTick = millis();
      mbState = MB_INSSTATUS;
      
      break;
    */
  }
}

#endif
